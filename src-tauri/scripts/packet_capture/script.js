...
function getSslSessionId(ssl) {
  try {
    if (!SSL_get_session || !SSL_SESSION_get_id) {
      return "";
    }
    var session = SSL_get_session(ssl);
    if (session == 0 || session.isNull()) {
      return "";
    }
...
Interceptor.attach(addresses["SSL_read"],
  {
    onEnter: function (args) {
      try {
        ...
        console.log("SSL_read onEnter error: " + e);
      }
    },
    onLeave: function (retval) {
      try {
        ...
        console.log("SSL_read onLeave error: " + e);
      }
    }
  });
...
Interceptor.attach(addresses["SSL_write"],
  {
    onEnter: function (args) {
      try {
        ...
        console.log("SSL_write onEnter error: " + e);
      }
    },
    onLeave: function (retval) {
    }
...
setTimeout(function() {
  if (typeof Java !== 'undefined' && Java.available) {
    ...
    Java.use("java.security.KeyStore$PrivateKeyEntry").getPrivateKey.implementation = function () {
      ...
      send(message, data.readByteArray(1))
      return result;
    }
